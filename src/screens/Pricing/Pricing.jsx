import React from "react";
import { Footer } from "../../components/Footer";
import { VuesaxBulkTick } from "../../components/VuesaxBulkTick";
import { VuesaxBulkPlayCircle } from "../../icons/VuesaxBulkPlayCircle";
import { VuesaxBulkTickCircle } from "../../icons/VuesaxBulkTickCircle";
import "../Pricing/pricingstyle.css";
import Navbar from "../Home/Navbar";
const Pricing = () => {
  return (
    <div className="index">
      <div className="div-2">
        <div className="frame-15">
          <div className="group-3">
            <div className="overlap-group-3">
              <p className="we-offer-great">
                We offer great prices, premium products
                and &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Services
                for your product
              </p>
              <div className="text-wrapper-15">Quality</div>
            </div>
          </div>
          <p className="text-wrapper-16">We have considered our solutions to support every stage of your growth.</p>
        </div>
        <div className="overlap">
          <div className="frame-16">
            <div className="frame-17">
              <div className="frame-18">
                <div className="frame-19">
                  <div className="text-wrapper-17">₹</div>
                  <div className="text-wrapper-18">0</div>
                  <div className="text-wrapper-19">/year</div>
                </div>
                <div className="text-wrapper-20">Free Plan</div>
              </div>
              <div className="frame-20">
                <div className="frame-21">
                  <VuesaxBulkTickCircle className="vuesax-bulk-tick" />
                  <div className="text-wrapper-21">Limited access to creatives</div>
                </div>
                <div className="frame-21">
                  <VuesaxBulkTickCircle className="vuesax-bulk-tick" />
                  <p className="text-wrapper-21">Filter by brand, keyword, industry, format</p>
                </div>
                <div className="frame-21">
                  <VuesaxBulkTickCircle className="vuesax-bulk-tick" />
                  <p className="text-wrapper-21">
                  No credit card required
                  </p>
                </div>
              </div>
              <div className="frame-23">
                <div className="frame-24">
                  <div className="text-wrapper-23">Current Plan</div>
                </div>
                <div className="text-wrapper-24">Cancel Anytime</div>
              </div>
            </div>
            <div className="frame-17">
              <div className="frame-18">
                <div className="frame-wrapper">
                  <div className="frame-19">
                    <div className="text-wrapper-17">₹</div>
                    <div className="text-wrapper-25">199</div>
                    <div className="text-wrapper-26">98</div>
                    <div className="text-wrapper-19">/Year</div>
                  </div>
                </div>
                <div className="text-wrapper-20">Pro Plan</div>
              </div>
              <div className="frame-20">
                <div className="frame-21">
                  <VuesaxBulkTickCircle className="vuesax-bulk-tick" />
                  <div className="text-wrapper-27">Get access to 3000+ creatives</div>
                </div>
                <div className="frame-21">
                  <VuesaxBulkTickCircle className="vuesax-bulk-tick" />
                  <div className="text-wrapper-27">Filter by brand, keyword, industry, format
                  </div>
                </div>
                
                <div className="frame-21">
                  <VuesaxBulkTickCircle className="vuesax-bulk-tick" />
                  <div className="text-wrapper-27">Wishlist and save</div>
                </div>
                
                
              </div>
              <div className="frame-21">
                <div className="frame-23">
                  <div className="frame-25">
                    <div className="text-wrapper-30">Upgrade</div>
                  </div>
                  <div className="text-wrapper-31">Cancel Anytime</div>
                </div>
              </div>
            </div>
          </div>
          <img
            className="img-3"
            alt="Img"
            src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/64088dfd259d863110d90801-doodle1-2.svg"
          />
        </div>
        <img
          className="img-4"
          alt="Img"
          src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/64088dfd259d863110d90801-doodle1-1.svg"
        />
        <div className="frame-26">
          <div className="text-wrapper-32">Trusted By Many</div>
          <div className="frame-27">
            <img
              className="image"
              alt=""
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64c5661df29788c65e4e3a24/img/image-2-1@2x.png"
            />
            <img
              className="image-2"
              alt=""
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64c5661df29788c65e4e3a24/img/image-3-1@2x.png"
            />
            <img
              className="image"
              alt=""
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/image-4@2x.png"
            />
            <img
              className="image-2"
              alt=""
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64c5661df29788c65e4e3a24/img/image-6@2x.png"
            />
            <img
              className="image"
              alt=""
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/image-5@2x.png"
            />
            <img
              className="image-3"
              alt=""
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64c5661df29788c65e4e3a24/img/image-7@2x.png"
            />
          </div>
        </div>
        <div className="frame-28">
          <div className="group-4">
            <div className="overlap-group-4">
              <p className="div-5">
                <span className="text-wrapper-33">Watch </span>
                <span className="text-wrapper-34">Demo</span>
              </p>
              <div className="text-wrapper-35">Demo</div>
            </div>
          </div>
          <div className="overlap-wrapper">
            <div className="overlap-2">
              <div className="image-wrapper">
                <img
                  className="image-4"
                  alt=""
                  src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/image-8.png"
                />
              </div>
              <VuesaxBulkPlayCircle className="vuesax-bulk-play" />
            </div>
          </div>
        </div>
        <div className="frame-29">
          <div className="group-wrapper">
            <div className="group-5">
              <div className="overlap-group-4">
                <p className="div-5">
                  <span className="text-wrapper-33">Inclued in our</span>
                  <span className="text-wrapper-34"> plans</span>
                </p>
                <div className="text-wrapper-36">Plans</div>
              </div>
            </div>
          </div>
          <div className="frame-30">
            <img
              className="image-5"
              alt=""
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/image-9.png"
            />
            <div className="frame-31">
              <p className="text-wrapper-37">Save Hours With Campaigns and Work With Influencers at Scale</p>
              <div className="frame-32">
                <div className="frame-33">
                  <div className="text-wrapper-38">Reason For choosing
</div>
                  <p className="text-wrapper-39">
                    Duis mollis gravida commodo id luctus erat porttitor ligula, eget lacinia odio sem aget elit.
                  </p>
                </div>
                <div className="frame-33">
                  <div className="text-wrapper-38">Reason For choosing
</div>
                  <p className="text-wrapper-39">
                    Duis mollis gravida commodo id luctus erat porttitor ligula, eget lacinia odio sem aget elit.
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="frame-34">
            <div className="frame-31">
              <p className="text-wrapper-37">Save Hours With Campaigns and Work With Influencers at Scale</p>
              <div className="frame-32">
                <div className="frame-33">
                  <div className="text-wrapper-38">Reason For choosing
</div>
                  <p className="text-wrapper-39">
                    Duis mollis gravida commodo id luctus erat porttitor ligula, eget lacinia odio sem aget elit.
                  </p>
                </div>
                <div className="frame-33">
                  <div className="text-wrapper-38">Reason For choosing
</div>
                  <p className="text-wrapper-39">
                    Duis mollis gravida commodo id luctus erat porttitor ligula, eget lacinia odio sem aget elit.
                  </p>
                </div>
              </div>
            </div>
            <img
              className="image-5"
              alt=""
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/image-9.png"
            />
          </div>
        </div>
        <div className="frame-35">
          <img
            className="line-2"
            alt="Line"
            src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/line-8.svg"
          />
          <div className="frame-36">
            <div className="frame-37">
              <div className="text-wrapper-40">?</div>
            </div>
            <div className="frame-38">
              <p className="text-wrapper-41">Can I cancel my subscription?</p>
              <p className="text-wrapper-42">
                Ut nunc, dui sit sit nisl, cras velit lorem. Laoreet gravida adipiscing augue sit justo elit. Mauris
                bibendum mattis et diam tellus. Auctor mauris felis lobortis tempus.Ut nunc, dui sit sit nisl, cras
                velit lorem. Laoreet gravida adipiscing augue sit justo elit. Mauris bibendum mattis et diam tellus.
                Auctor mauris felis lobortis tempus.Ut nunc, dui sit sit nisl, cras velit lorem. Laoreet gravida
                adipiscing augue sit justo elit. Mauris bibendum mattis et diam tellus. Auctor mauris felis lobortis
                tempus.
              </p>
            </div>
            <img
              className="vuesax-bulk-tick"
              alt="Vuesax linear add"
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/vuesax-linear-add.svg"
            />
          </div>
          <img
            className="line-3"
            alt="Line"
            src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/line-8.svg"
          />
          <div className="frame-36">
            <div className="frame-37">
              <div className="text-wrapper-40">?</div>
            </div>
            <div className="frame-33">
              <p className="text-wrapper-41">Can I cancel my subscription?</p>
              <p className="text-wrapper-42">
                Ut nunc, dui sit sit nisl, cras velit lorem. Laoreet gravida adipiscing augue sit justo elit. Mauris
                bibendum mattis et diam tellus. Auctor mauris felis lobortis tempus.Ut nunc, dui sit sit nisl, cras
                velit lorem. Laoreet gravida adipiscing augue sit justo elit. Mauris bibendum mattis et diam tellus.
                Auctor mauris felis lobortis tempus.Ut nunc, dui sit sit nisl, cras velit lorem. Laoreet gravida
                adipiscing augue sit justo elit. Mauris bibendum mattis et diam tellus. Auctor mauris felis lobortis
                tempus.
              </p>
            </div>
            <img
              className="vuesax-bulk-tick"
              alt="Vuesax linear add"
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/vuesax-linear-add-1.svg"
            />
          </div>
          <img
            className="line-3"
            alt="Line"
            src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/line-8.svg"
          />
          <div className="frame-36">
            <div className="frame-37">
              <div className="text-wrapper-40">?</div>
            </div>
            <div className="frame-38">
              <p className="text-wrapper-41">Can I cancel my subscription?</p>
              <p className="text-wrapper-42">
                Ut nunc, dui sit sit nisl, cras velit lorem. Laoreet gravida adipiscing augue sit justo elit. Mauris
                bibendum mattis et diam tellus. Auctor mauris felis lobortis tempus.Ut nunc, dui sit sit nisl, cras
                velit lorem. Laoreet gravida adipiscing augue sit justo elit. Mauris bibendum mattis et diam tellus.
                Auctor mauris felis lobortis tempus.Ut nunc, dui sit sit nisl, cras velit lorem. Laoreet gravida
                adipiscing augue sit justo elit. Mauris bibendum mattis et diam tellus. Auctor mauris felis lobortis
                tempus.
              </p>
            </div>
            <img
              className="vuesax-bulk-tick"
              alt="Vuesax linear add"
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/vuesax-linear-add.svg"
            />
          </div>
          <img
            className="line-3"
            alt="Line"
            src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/line-8.svg"
          />
          <div className="frame-36">
            <div className="frame-37">
              <div className="text-wrapper-40">?</div>
            </div>
            <div className="frame-38">
              <p className="text-wrapper-41">Can I cancel my subscription?</p>
              <p className="text-wrapper-42">
                Ut nunc, dui sit sit nisl, cras velit lorem. Laoreet gravida adipiscing augue sit justo elit. Mauris
                bibendum mattis et diam tellus. Auctor mauris felis lobortis tempus.Ut nunc, dui sit sit nisl, cras
                velit lorem. Laoreet gravida adipiscing augue sit justo elit. Mauris bibendum mattis et diam tellus.
                Auctor mauris felis lobortis tempus.Ut nunc, dui sit sit nisl, cras velit lorem. Laoreet gravida
                adipiscing augue sit justo elit. Mauris bibendum mattis et diam tellus. Auctor mauris felis lobortis
                tempus.
              </p>
            </div>
            <img
              className="vuesax-bulk-tick"
              alt="Vuesax linear add"
              src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/vuesax-linear-add.svg"
            />
          </div>
          <img
            className="line-3"
            alt="Line"
            src="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64d53bb23acc02a59b7cfa6b/img/line-8.svg"
          />
        </div>
        <Footer
          className="footer-instance"
          img="https://anima-uploads.s3.amazonaws.com/projects/64941b1b7b1311a96e83e640/releases/64c5661df29788c65e4e3a24/img/-.svg"
        />
        <Navbar/>
      </div>
    </div>
  );
};
export default Pricing
